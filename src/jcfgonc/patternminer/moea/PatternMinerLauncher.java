...
public class PatternMinerLauncher {
	...
	private static void testQueryLoop(KnowledgeBase kb) throws NoSuchFileException, IOException, InterruptedException {
		while (true) {
			for (int i = 0; i < 32; i++) {
				testQuery(
						"X3,definedas,X6;X1,antonym,X4;X2,influencedby,X0;X0,field,X3;X2,knownfor,X5;X1,hascontext,X3;X2,field,X3;X0,knownfor,X5;X4,hascontext,X3;\r\n",
						kb);
	...
	private static void testQuery(String query, KnowledgeBase kb) throws IOException, NoSuchFileException {
		...
		Ticker t = new Ticker();
		BigInteger matches = kb.count(q, PatternMinerConfig.BLOCK_SIZE, PatternMinerConfig.PARALLEL_LIMIT, null, true,
