...
public class MutationCode {
	...
	public static void mutation(final StringGraph kbGraph, final RandomGenerator random, StringGraph pattern, final boolean forceAdd) {
		...
		if (addEdge) { // add
			...
			pattern.removeEdge(VariousUtils.getRandomElementFromCollection(edgesFrequent, random));
		}
	...
	private static boolean mutationMakeCycle(final StringGraph kbGraph, final RandomGenerator random, StringGraph pattern) {
		if (pattern.numberOfVertices() >= 3 /* && random.nextFloat() > 0.5 */) {
			...
			VariousUtils.shuffleArrayList(vertices, random);
	...
	private static void mutationAddRandomEdge(final StringGraph kbGraph, final RandomGenerator random, StringGraph pattern) {
		pattern.addEdge(VariousUtils.getRandomElementFromCollection(kbGraph.edgeSet(), random));
	}
	...
	private static Object2IntOpenHashMap<String> filterMapKeys(Object2IntOpenHashMap<String> relationCount, Set<String> labels) {
		...
		patternRelationCountFiltered.defaultReturnValue(-1);
	...
	private static void addRelationWithRarestLabel(final RandomGenerator random, StringGraph pattern,
			Object2IntOpenHashMap<String> patternRelationCount, Set<StringEdge> kbEdges) {
		...
		{
			Set<StringEdge> subtract = VariousUtils.subtract(edgesWithDifferentLabels, pattern.edgeSet());
			if (!subtract.isEmpty()) {
				StringEdge edge = VariousUtils.getRandomElementFromCollection(subtract, random);
...
//				System.out.println(rarestLabelEdges); // []
			...
			Set<StringEdge> subtract = VariousUtils.subtract(rarestLabelEdges, pattern.edgeSet());
			if (!subtract.isEmpty()) {
				StringEdge edge = VariousUtils.getRandomElementFromCollection(subtract, random);
				pattern.addEdge(edge);
			}
